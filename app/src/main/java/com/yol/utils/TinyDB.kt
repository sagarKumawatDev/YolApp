...
class TinyDB     //preferences = PreferenceManager.getDefaultSharedPreferences(appContext);
    ...
    fun setUserDetails(model: UserProfileDetails?) {
        val editor: SharedPreferences.Editor = preferences.edit()
        val gson = Gson()
        val json = gson.toJson(model)
        editor.putString(Constants.SharedPref.USER_PROFILE_DETAIL, json)
        editor.apply()
    }

    fun getUserDetails(): UserProfileDetails? {
        val gson = Gson()
        val json: String? = preferences.getString(Constants.SharedPref.USER_PROFILE_DETAIL, "")
        return gson.fromJson(json, UserProfileDetails::class.java)
    }
